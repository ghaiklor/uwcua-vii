...
GitterBot.prototype = {
   ...
   * @returns {GitterBot}
   */
  setExecPattern: function (pattern) {
    this._execPattern = pattern ? new RegExp(pattern) : (process.env['GITTER_BOT_PATTERN'] || localConfig.execPattern || /^calc\s*/);
    return this;
  },
  ...
  _onRoomMessage: function (room, message) {
    ...
    if (pattern.test(text)) {
      room.send(text.replace(pattern, ''));
    }
